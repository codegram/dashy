defmodule DashyWeb.UILive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok, socket |> assign(uses_fake_data: true)}
  end
  ...
  def handle_event("update-runs", _, socket) do
    runs = get_module(socket).runs()
    data = build_labels(runs)
    {:noreply, socket |> push_event("load-runs", %{data: data})}
  end

  def handle_event("update-parts", _, socket) do
    # data = get_module(socket).parts(count: 50)
    data = Dashy.Charts.WorkflowRunsFake.parts(count: 50)
  ...
  def handle_info(:load_parts, socket) do
    # data = get_module(socket).parts(count: 15)
    data = Dashy.Charts.WorkflowRunsFake.parts(count: 15)
    {:noreply, socket |> push_event("load-parts", %{data: data})}
  end
  ...
  defp get_module(%{assigns: %{uses_fake_data: true}} = _socket),
    do: Dashy.Charts.WorkflowRunsFake

  defp get_module(%{assigns: %{uses_fake_data: false}} = _socket),
